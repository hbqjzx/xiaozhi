<template>
	<view class="container">
		<image class="logo" src="https://www.***.com/api/apiimg/xiaozhi.png"></image>
		<view class="text-area">
			<text class="title">{{title}}</text>
		</view>
		<view class="form-item">
			<text class="label">父姓</text>
		    <input type="text" class="input" v-model="inputValue" placeholder="请输入父姓" />				
		</view>
		<view class="form-item">
			<text class="label">母姓</text>
			<input id="user-text" class="input" v-model="motherSurname" placeholder="请输入母姓" />	
		</view>
		<view class="sex_wapper">
			<view class="flex1 boy" :class="sexIndex==0?'active':''" data-index="0" @click="checkSex">
				<view class="sex"></view>
				<view class="txt">男宝</view>
			</view>
			<view class="flex1 girls" :class="sexIndex==1?'active':''" data-index="1" @click="checkSex">
				<view class="sex"></view>
				<view class="txt">女宝</view>
			</view>

		</view>		
		<button class="button" @click="startNaming($event)">开始取名</button>		
		<!-- 广告开始 -->
		<view class="myad">
...
export default {
  ...
  methods: {	 
	   ...
	   onShareAppMessage() {
	       let that=this;	       
		   var url="/pages/index/index";//你的转发页面路径拼接参数
	       return {
	           title: "AI小智取名",
	           path: url,
	       }
	   ...
	   //分享朋友圈
	    onShareTimeline() {
	           let that=this;	           
			   var url="/pages/index/index";//你的分享页面路径
	           return {
	                title: 'AI小智取名',
	                path: url,	                
	                imageUrl: 'https://www.***.com/api/apiimg/xiaozhi.png'//你分享的封面
	           };
	        },
    // 选择性别
    checkSex: function(e) {
    ...
    startNaming() {      
	  ...
	  const value = this.inputValue; // 获取输入框中的值	
	  ...
	  const apiUrl = `/pages/Result/Result?msg=${value}&sex=${sex}`; // 将值拼接到API的参数字符串中	  
	   	uni.navigateTo({
	   	url:apiUrl
	   })		
    },	
